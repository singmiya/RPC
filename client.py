...
zk_root = '/demo'
G = {'servers': None}  # 全局变量，RPCClient对象列表
...
def get_servers():
    ...
    def watch_servers(*args):  # 闭包函数
        ...
        for child in zk.get_children(zk_root, watct=watch_servers):
        ...
        for addr in add_addrs:
            G['servers'].append(RPCClient(addr))
    ...
    G['servers'] = [RPCClient(s) for s in current_addrs]
    return G['servers']
